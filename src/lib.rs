...
trait Register {
    async fn read<I2C>(&self, i2c: &mut I2C, reg_addr: u8) -> Result<u8, I2C::Error>
    where
        I2C: embedded_hal_async::i2c::I2c,
    {
        let mut data: [u8; 1] = [0];
        i2c.write_read(I2C_ADDRESS, &[reg_addr], &mut data).await?;
        Ok(data[0])
    }

    async fn write<I2C>(&self, i2c: &mut I2C, reg_addr: u8, bits: u8) -> Result<(), I2C::Error>
    where
        I2C: embedded_hal_async::i2c::I2c,
    {
        i2c.write(I2C_ADDRESS, &[reg_addr, bits]).await
    }
}
...
impl Iis2mdc {
    pub async fn new<I2C>(i2c: &mut I2C) -> Result<Iis2mdc, I2C::Error>
    where
        I2C: embedded_hal_async::i2c::I2c,
    {
        let mut registers = [0u8; 3];
        i2c.write_read(I2C_ADDRESS, &[0x60], &mut registers).await?;
        ...
        I2C: embedded_hal_async::i2c::I2c,
    {
        ...
        i2c.write_read(I2C_ADDRESS, &[0x6e], &mut measurements).await?;
    ...
    pub async fn get_measurements<I2C>(&mut self, i2c: &mut I2C) -> Result<[f64; 3], I2C::Error>
    where
        I2C: embedded_hal_async::i2c::I2c,
    {
        let mut measurements = [0u8; 6];
        i2c.write_read(I2C_ADDRESS, &[0x68], &mut measurements).await?;
